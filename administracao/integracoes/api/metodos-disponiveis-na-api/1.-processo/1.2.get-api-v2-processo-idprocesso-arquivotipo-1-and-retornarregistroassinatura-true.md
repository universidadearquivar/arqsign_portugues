...
# ✔️ 1.2.GET/api/v2/processo/{idProcesso}?arquivoTipo =1\&retornarRegistroAssinatura=true

#### Objetivo do Método

Este serviço permite aos Clientes, via API, buscar um processo, **não excluído logicamente**, com seus respectivos dados do processo, signatários e documentos do processo.
...
**Descrição:** Parâmetro opcional informando se é para retornar o **link para download ou base64 dos documentos.**

**Formato:** Byte - 1 = base64, 2 = Link para download&#x20;

**Requerido**: <mark style="color:blue;">**Não**</mark>&#x20;

> Quando **não enviado** ou for **enviado valor diferente de 1 ou 2**, significa que o sistema deve considerar o valor default. Ou seja, **?arquivoTipo =1, retornando os documentos do processo em formato base64.**
>
> Quando for o **valor 1** no parâmetro **arquivoTipo** , significa que o sistema retorna os documentos do processo de assinaturas em **formato base64**.&#x20;
>
> Quando for enviado o **valor 2** no parâmetro **arquivoTipo**, significa que o sistema **retorna o link para download dos documentos** do processo de assinaturas.&#x20;
>
> > **Atenção:** O sistema retorna o link para download ou o arquivo base64. Não retornar ambos os formatos.&#x20;

#### <sup>B</sup>retornarRegistroAssinatura

**Descrição:** Parâmetro opcional informando se retorna o registro de assinatura de cada documento do processo.

**Formato:** Booleano – true ou false

**Requerido:** <mark style="color:blue;">**Não**</mark>&#x20;

> Quando **não enviado** ou for enviado valor **true** ou for enviado valor **diferente de true ou false**, o sistema considera o valor default. Ou seja, **retornar o registro de assinatura dos respectivos documentos do processo.**
>
> &#x20;Quando for o valor **false** no parâmetro **retornarRegistroAssiatura**, significa que o sistema **não retorna o registro de assinatura** dos respectivos documentos do processo.
...
<figure><img src="../../../../../.gitbook/assets/Contract (2) (3).jpg" alt=""><figcaption></figcaption></figure>
...
O sistema retorna o caminho da pasta do processo. Para um documento com status diferente de Concluído, se a pasta estiver excluída, deve-se retornar o caminho da pasta raiz da conta, que é onde o documento será salvo quando for concluído.
...
O sistema retorna o id do status do processo **(1 - Criado, 2 - Aguardando,3 - Em processo, 4 - Concluído ou 5 - Cancelado).**&#x20;
...
O sistema retorna o id tipo de ação do signatário no processo que pode ser **Assina Online** ou **Receber uma cópia**.&#x20;
...
O sistema deve retornar à informação se houve falha no envio do código de segurança:  &#x20;
...
*   **idTipoAssinatura** &#x20;
    ...
    O sistema retorna o id do tipo de assinatura que o signatário executou ou executará. Este parâmetro pode ter o valor: **1** (Assinatura Eletrônica), ou **2** (Certificado Digital - ICP Brasil) ou **4** (Certificado Digital – Outros)&#x20;

    Quando o **tipo de ação** do signatário for igual a **Receber uma cópia**, o parâmetro **idTipoAssinatura** será **null**.&#x20;
* **dataAssinatura**
...
O sistema retorna o link do documento compartilhado por QRCode.&#x20;

O link compartilhado do documento é gerado quando o processo com a flag GerarQRCode = true é concluído.

**linkParaBaixarDocumento** &#x20;

O sistema retorna o link para baixar o documento, conforme o parâmetro **arquivoTipo** na requisição.

**base64Documento**

O sistema retorna o base64 do arquivo.

**retornarRegistroAssinaturas**

O sistema retorna, para cada documento do processo, o respectivo registro de assinaturas, conforme o parâmetro **registro assinatura informado ou não na requisição.**
